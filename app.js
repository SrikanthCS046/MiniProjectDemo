// app.js
// Consolidated, corrected server for GrowAi with a focused Practice page flow.
// - Place this file at the project root and run `node app.js`
// - Ensure `public/models` contains face-api.js weights (for client-side face detection)
// - Ensure questions JSON exists at ./questions/roles.json (fallback provided)

// -------------------------------------
// Load Environment Variables & Packages
// -------------------------------------
...
// local helpers (already in your project)
...
const Session = require("./model/session.js"); // optional: used to save practice sessions

// -------------------------------------
// Questions (role -> array of questions)
// -------------------------------------
let questions = {};
try {
  ...
  if (fs.existsSync(qpath)) {
    ...
    console.log("questions/roles.json not found — using built-in fallback questions.");
  }
} catch (err) {
...
// -------------------------------------
// Passport (ensure config/passport.js exports a function that takes passport)
// -------------------------------------
...
// -------------------------------------
// Validation schemas (your existing)
 // -------------------------------------
...
// -------------------------------------
// Utility: skills database & extractor (kept from you)
// -------------------------------------
...
// -------------------------------------
// ROUTES - existing pages (unchanged)
// -------------------------------------
...
// -------------------------------------
// PRACTICE PAGE (render with roles + companies)
// -------------------------------------
...
// -------------------------------------
// PRACTICE: get next question for a role
// POST /next-question { role, index }
// -------------------------------------
...
// -------------------------------------
// PRACTICE: analyze raw answer (simple endpoint used in flow)
// POST /analyze-answer { answer, durationSec }
// returns meta: words, wpm, fillerCount, sentimentScore
// -------------------------------------
...
// -------------------------------------
// PRACTICE: full analysis endpoint used by client
// Accepts: transcript, durationSec, words, fillerCount, avgVolume, expressions
// Returns: scores, strengths, suggestions
...
app.post("/api/practice/analyze", async (req, res) => {
  try {
    ...
    const idealWPM = 130;
    const wpmScore = Math.max(0, 100 - Math.abs(idealWPM - wpm)); // 0..100

    const fillerPenalty = Math.max(0, 100 - fillerCount * 8); // higher is better
    ...
    // facial expression score from expressions object (happy, neutral, sad, angry...)
    const happy = expressions.happy || 0;
    const neutral = expressions.neutral || 0;
    const sad = expressions.sad || 0;
    const angry = expressions.angry || 0;
    // compute a rough normalized score
    const exprRaw = (happy * 1.4 + neutral * 1.0) - (sad * 0.5 + angry * 0.6);
    let exprScore = Math.round(Math.min(1, Math.max(-1, exprRaw)) * 100); // -100..100 -> clamp
    // convert to 0..100
    const facialScore = Math.max(0, Math.min(100, exprScore));

    // sentiment/tone
    const sentimentResult = sentiment.analyze(transcript || "");
    let toneScore = 50 + sentimentResult.score * 5; // center at 50, adjust by sentiment
    toneScore = Math.max(0, Math.min(100, Math.round(toneScore)));

    // communication score: weighted blend
    ...
    // suggestions and strengths (simple rule-based)
    const suggestions = [];
    ...
    // optionally save session to DB (if you want full history)
    try {
      if (Session) {
...
// -------------------------------------
// FINAL SUMMARY: aggregates results from client
// POST /final-summary { allAnswers: [ { sentimentScore, fillerCount, ... } ] }
// -------------------------------------
...
// -------------------------------------
// RESUME analysis routes (kept from your code, corrected PDF usage)
// -------------------------------------
...
// -------------------------------------
// Google OAuth (leave as-is if configured)
// -------------------------------------
